...
class single_word(object):
    ...
    def updateH(self, maxTF, avgTF, stdTF, number_of_sentences, features=None):
        ...
        tagged_term = pos_tag([self.unique_term])[0][1]
        print("Post Tagger**********")
        print("["+self.unique_term+"]"+"["+tagged_term+"]")

        TPos = 0.

        if tagged_term == "NN":
            TPos = 1
        elif tagged_term == "NNP":
            TPos = 1
        elif tagged_term == "NNPS":
            TPos = 1
        elif tagged_term == "NNS":
            TPos = 1
        elif tagged_term == "JJ":
            TPos = .5
        elif tagged_term == "VB":
        ...
        self.H = self.WPos / (self.WCase + self.WFreq + self.WSpread + TPos)
